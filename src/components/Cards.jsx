      ...
      <div className="max-w-8xl mx-auto">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
          {content.map(item => (
            <div
              key={item.id}
              className={`relative p-5 sm:p-6 rounded-xl bg-gradient-to-br ${getCategoryColor(item.category)} border backdrop-blur-sm hover:scale-[1.03] transition-all duration-300 hover:shadow-[0_10px_30px_rgba(236,72,153,0.4)]`}
            >
              {/* Header */}
              ...
              {item.followup && (
                <div className="bg-black/30 rounded-lg p-3 mb-4 border border-fuchsia-500/10">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <button
                      onClick={() => handleCopy(item.followup, `${item.id}-followup`)}
                      className="p-1 rounded hover:bg-fuchsia-500/20 transition-colors"
                    >
